...
import { CATEGORY_OPTIONS, SUBCATEGORY_OPTIONS } from './constants';
...
export async function createProduct(data: Partial<AdminProduct>): Promise<AdminProduct> {
  ...
  const product: AdminProduct = {
    ...
    category_name: data.category_name ?? CATEGORY_OPTIONS.find((c) => c.id === data.category_id)?.name ?? '',
    subcategory_id: data.subcategory_id ?? null,
    subcategory_name: data.subcategory_name ?? (
      data.subcategory_id
        ? (SUBCATEGORY_OPTIONS[data.category_id ?? ''] ?? []).find((s) => s.id === data.subcategory_id)?.name ?? null
        : null
    ),
